...
bool token_analize(char* token, const int str_number, LT::LexTable& lexTable, IT::IdTable& idTable)
{
	...
	case '/':
	{
		Add(lexTable, { LEX_DIRSLASH,str_number,LT_TI_NULLXDX,'/' });
		return true;
	}
	...
	case '\'':
	{
		...
		if (FST::execute(*string_literal))
		{
			...
			{
				idTable.Add({ "\0", (idTable.GetEntry(lexTable.GetEntry(lexTable.size - 2).idxTI).id[0] != '\0' && lexTable.GetEntry(lexTable.size - 1).lexema == '=') ? idTable.GetEntry(lexTable.GetEntry(lexTable.size - 2).idxTI).id : "-" , IT::IDDATATYPE::STR, IT::IDTYPE::L });
	...
	default:
	{
		...
		if (FST::execute(*number_literal))
		{
			...
			{
				idTable.Add({ "\0", (idTable.GetEntry(lexTable.GetEntry(lexTable.size - 2).idxTI).id[0] != '\0' && lexTable.GetEntry(lexTable.size - 1).lexema == '=') ? idTable.GetEntry(lexTable.GetEntry(lexTable.size - 2).idxTI).id : "-" , IT::IDDATATYPE::INT, IT::IDTYPE::L });
